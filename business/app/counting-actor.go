...
import (
	...
	"github.com/dumacp/go-counterpass/business/doors"
...
func NewCountingActor(listenner *listen.ListenActor) *CountingActor {
	count := &CountingActor{}
	...
	case *actor.Started:
		...
		if a.listenner != nil {
			if !a.disableDoorGpio {
				props3 := actor.PropsFromProducer(func() actor.Actor { return &doors.DoorsActor{} })
	...
	case *messages.Event:
		...
		case messages.INPUT:
			...
			if diff > 0 && a.rawInputs[id] <= 0 {
				...
				ctx.Send(a.listen, &listen.MsgLogRequest{})
...
func registersMap(inputs, outputs map[int32]int64, tp int) *pubs.RegisterMap {
	reg := &pubs.RegisterMap{}
...
func registers(inputs, outputs map[int32]int64, tp int) *pubs.Register {
	...
	if tp == 2 {
		...
		sortInputs := utils.SortMap(inputs)
		for _, v := range sortInputs {
			regv = append(regv, v)
		}
		sortOutputs := utils.SortMap(outputs)
